# Building an eCommerce web app with Temporal, Part 3: Testing
...
Below is a basic setup for testing a Temporal workflow using `go test` and [Testify](https://github.com/stretchr/testify) based on [Temporal's Go testing docs](https://docs.temporal.io/docs/go/testing).
...
For example, Temporal Workflows time out after [10 years by default](https://docs.temporal.io/docs/concepts/workflows/#execution-timeout), so the previous examples would run for over a decade if the test environment used wall-clock time!
